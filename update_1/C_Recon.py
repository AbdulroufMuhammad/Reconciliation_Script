    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        ...
        summary_data = {
            'Metric': [
                '═══ BANK STATEMENT ═══',
                'Total Bank records',
                'Matched in Stage 1 (Primary Ledger)',
                'Matched in Stage 2 (Secondary Ledger)',
                'Still Unmatched after both stages',
                'Overall Match Rate',
                '',
                '═══ PRIMARY LEDGER (Stage 1) ═══',
                'Total Ledger 1 records',
                'Matched with Bank',
                'Unmatched',
                '',
                '═══ SECONDARY LEDGER (Stage 2) ═══',
            ...
            'Value': [
                ...
                f"{(matched_stage1_count + matched_stage2_count)/total_bank*100:.2f}%",
                '',
                '',
                len(ledger1_df),
                len(matched_ledger1),
                len(ledger1_df) - len(matched_ledger1),
                '',
                '',
                len(ledger2_df),
                len(matched_ledger2),
                len(ledger2_df) - len(matched_ledger2)
            ]
        }
